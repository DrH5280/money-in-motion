...
function Para(p){
  ...
  return <p style={{fontFamily:"'DM Sans',sans-serif",fontWeight:300,fontSize:16,color:C.slate,lineHeight:1.9,marginBottom:20}}>{parts}</p>;
}
      ...
      {Object.keys(props.presets).map(function(name){
        ...
        return(
          <button key={name} onClick={function(){props.onSelect(name);}}
            style={{padding:"5px 12px",borderRadius:16,border:"1.5px solid "+(active?C.teal:"rgba(28,31,46,0.18)"),background:active?C.teal:"transparent",color:active?C.slate:C.slate,fontFamily:"'DM Sans',sans-serif",fontSize:11,fontWeight:active?500:300,cursor:"pointer",transition:"all 0.18s",opacity:active?1:0.7}}>
            {name}
          </button>
        );
      })}
...
function NI(props){
  return(
    <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",padding:"7px 0",borderBottom:"1px solid rgba(28,31,46,0.05)"}}>
      <div>
        <div style={{fontFamily:"'DM Sans',sans-serif",fontSize:13,color:C.slate,fontWeight:300,opacity:props.dim?0.45:1}}>{props.label}</div>
      ...
      <div style={{display:"flex",alignItems:"center",gap:3,marginLeft:8,flexShrink:0}}>
        <span style={{fontFamily:"'DM Sans',sans-serif",fontSize:12,color:C.slate,opacity:0.3}}>$</span>
        <input type="number" value={props.value||""} onChange={function(e){props.onChange(Number(e.target.value)||0);}}
          style={{width:100,textAlign:"right",padding:"4px 6px",borderRadius:6,border:"1.5px solid rgba(28,31,46,0.12)",background:C.ivory,fontFamily:"'DM Sans',sans-serif",fontSize:12,color:C.slate}}/>
      </div>
    </div>
  );
}
...
function Chip(props){
  ...
  return(
    <div style={{background:bg,borderLeft:"3px solid "+col,borderRadius:"0 7px 7px 0",padding:"8px 11px",marginTop:6}}>
      ...
      <div style={{fontFamily:"'DM Sans',sans-serif",fontSize:11,fontWeight:300,color:C.slate,lineHeight:1.55}}>{props.children}</div>
    </div>
  );
}
...
function SH(props){return <div style={{fontFamily:"'Syne',serif",fontWeight:600,fontSize:12,color:props.color||C.slate,marginBottom:2,marginTop:props.top?12:0}}>{props.children}</div>;}
function LR(props){return(<div style={{display:"flex",justifyContent:"space-between",padding:"2px 11px"}}><span style={{fontFamily:"'DM Sans',sans-serif",fontSize:11,color:C.slate,opacity:0.58}}>{props.label}</span><span style={{fontFamily:"'DM Sans',sans-serif",fontSize:11,color:C.slate,fontVariantNumeric:"tabular-nums"}}>{props.paren?"("+fmt(Math.abs(props.value))+")":fmt(props.value)}</span></div>);}
  ...
  return(
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",height:"100%",overflow:"hidden"}}>
      <div style={{overflowY:"auto",padding:"20px 22px",borderRight:"1px solid "+C.dark}}>
        ...
        <p style={{fontFamily:"'DM Sans',sans-serif",fontSize:11,color:C.slate,opacity:0.45,marginBottom:14,lineHeight:1.55}}>
      ...
      <div style={{overflowY:"auto",padding:"20px 18px"}}>
        ...
        <div style={{background:"white",border:"1px solid "+C.dark,borderRadius:10,overflow:"hidden",marginBottom:12}}>
          <div style={{background:C.dark,padding:"9px 11px"}}><div style={{fontFamily:"'Syne',serif",fontWeight:600,fontSize:12,color:C.slate}}>Profit & Loss Statement</div><div style={{fontFamily:"'DM Sans',sans-serif",fontSize:10,color:C.slate,opacity:0.4,marginTop:1}}>{preset} · All figures USD</div></div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",height:"100%",overflow:"hidden"}}>
      ...
      <div style={{overflowY:"auto",padding:"20px 18px"}}>
        ...
        <div style={{background:bal?TF:RF,border:"2px solid "+(bal?C.teal:C.red),borderRadius:8,padding:"9px 12px",marginBottom:10,display:"flex",alignItems:"center",gap:9}}>
          <span style={{fontSize:15}}>{bal?"✓":"✗"}</span>
          <div>
            <div style={{fontFamily:"'DM Sans',sans-serif",fontSize:12,fontWeight:500,color:bal?C.teal:C.red}}>{bal?"Equation Balanced":"Out of Balance — Adjust Equity"}</div>
        ...
        <div style={{background:"white",border:"1px solid "+C.dark,borderRadius:10,overflow:"hidden",marginBottom:10}}>
          <div style={{background:C.dark,padding:"9px 11px"}}><div style={{fontFamily:"'Syne',serif",fontWeight:600,fontSize:12,color:C.slate}}>Balance Sheet</div><div style={{fontFamily:"'DM Sans',sans-serif",fontSize:10,color:C.slate,opacity:0.4,marginTop:1}}>{preset} · As of Today</div></div>
          <div style={{padding:"5px 0"}}>
  ...
  return(
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",height:"100%",overflow:"hidden"}}>
      ...
      <div style={{overflowY:"auto",padding:"20px 18px"}}>
        ...
        <div style={{background:"white",border:"1px solid "+C.dark,borderRadius:10,overflow:"hidden",marginBottom:10}}>
          <div style={{background:C.dark,padding:"9px 11px"}}><div style={{fontFamily:"'Syne',serif",fontWeight:600,fontSize:12,color:C.slate}}>Statement of Cash Flows</div><div style={{fontFamily:"'DM Sans',sans-serif",fontSize:10,color:C.slate,opacity:0.4,marginTop:1}}>{preset} · For the Period</div></div>
          <div style={{padding:"5px 0"}}>
            ...
            <div style={{display:"flex",justifyContent:"space-between",padding:"2px 11px"}}><span style={{fontFamily:"'DM Sans',sans-serif",fontSize:11,color:C.slate,opacity:0.55}}>Net Change in Cash</span><span style={{fontFamily:"'DM Sans',sans-serif",fontSize:11,color:net>=0?C.green:C.red,fontWeight:500,fontVariantNumeric:"tabular-nums"}}>{fmt(net)}</span></div>
            <Dv/>
  ...
  return(
    <div style={{background:"white",border:"1px solid #d0cfc9",borderRadius:4,overflow:"hidden",fontFamily:"'DM Sans',sans-serif",marginBottom:props.mb||0}}>
      ...
      <div style={{borderBottom:"2px solid "+C.slate,padding:"18px 24px 14px",textAlign:"center"}}>
        <div style={{fontFamily:"'Syne',serif",fontWeight:600,fontSize:17,color:C.slate,letterSpacing:"0.01em"}}>{props.company}</div>
        <div style={{fontSize:14,fontWeight:500,color:C.slate,marginTop:3}}>{props.title}</div>
        <div style={{fontSize:11,color:"#666",marginTop:2}}>{props.period}</div>
      </div>
...
function SectionHeader(props){
  return(
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline",padding:"12px 24px 3px",borderTop:props.borderTop?"1px solid #e8e6e0":undefined,marginTop:props.borderTop?10:0}}>
      <span style={{fontFamily:"'DM Sans',sans-serif",fontSize:12,fontWeight:500,color:C.slate,textTransform:"uppercase",letterSpacing:"0.06em"}}>{props.label}</span>
      {props.right&&<span style={{fontFamily:"'DM Sans',sans-serif",fontSize:11,fontWeight:500,color:"#666",textTransform:"uppercase",letterSpacing:"0.06em"}}>{props.right}</span>}
    </div>
  );
}
...
function LineItem(props){
  ...
  return(
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline",padding:"2px 24px",paddingLeft:(24+indent*18)+"px",background:props.shaded?"#faf9f6":undefined}}>
      <span style={{fontFamily:"'DM Sans',sans-serif",fontSize:13,color:props.muted?"#888":C.slate,fontStyle:props.italic?"italic":undefined,fontWeight:props.bold?500:300}}>{props.label}</span>
      {props.value!==undefined&&<span style={{fontFamily:"'DM Sans',sans-serif",fontSize:13,fontWeight:props.bold?500:300,color:col,fontVariantNumeric:"tabular-nums",minWidth:90,textAlign:"right"}}>{typeof props.value==="number"?fmt(props.value):props.value}</span>}
    </div>
  );
}
...
function TotalLine(props){
  ...
  return(
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline",padding:"5px 24px",borderTop:"1px solid "+(props.doubleLine?"#999":"#ccc"),borderBottom:props.doubleLine?"3px double #999":undefined,marginTop:props.mt||4,marginBottom:props.mb||0,background:props.highlighted?"rgba(0,212,180,0.06)":undefined}}>
      <span style={{fontFamily:"'DM Sans',sans-serif",fontSize:13,fontWeight:600,color:props.color||C.slate}}>{props.label}</span>
...
function NoteLine(props){
  return(
    <div style={{display:"flex",justifyContent:"space-between",padding:"2px 24px"}}>
      <span style={{fontFamily:"'DM Sans',sans-serif",fontSize:11,color:"#888",fontStyle:"italic"}}>{props.label}</span>
      <span style={{fontFamily:"'DM Sans',sans-serif",fontSize:11,color:C.teal,fontWeight:500,minWidth:90,textAlign:"right"}}>{props.value}</span>
    </div>
  );
}
  ...
  return(
    <StatementShell company="[Amazon FBA Business] — Your 30% Share" title="Profit & Loss (Allocated)" period="July 1 – December 31, 2024 (6 Months)" mb={0}>
      <div style={{margin:"8px 24px 0",padding:"8px 12px",background:"rgba(0,212,180,0.06)",borderRadius:4,border:"1px solid rgba(0,212,180,0.2)"}}>
        <span style={{fontFamily:"'DM Sans',sans-serif",fontSize:11,color:C.teal,fontWeight:400}}>This statement reflects your 30% equity allocation. Full business figures are noted in italics.</span>
      </div>
      <Spacer/>
      <SectionHeader label="Income"/>
        ...
        <div>
          ...
          <div style={{display:"flex",justifyContent:"center",padding:"6px 24px"}}>
            <div style={{display:"flex",alignItems:"center",gap:8,background:"rgba(0,212,180,0.08)",borderRadius:4,padding:"5px 12px"}}>
              ...
              <span style={{fontFamily:"'DM Sans',sans-serif",fontSize:11,color:C.teal,fontWeight:500}}>Assets = Liabilities + Equity ({fmt(ta)} = {fmt(tl)} + {fmt(equity)})</span>
            </div>
          </div>
        </div>
      </div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12}}>
          {[
            ...
            <div key={r.label} style={{background:"#faf9f6",borderRadius:4,padding:"10px 12px",border:"1px solid #e8e6e0"}}>
              ...
              <div style={{fontFamily:"'Syne',serif",fontWeight:600,fontSize:18,color:r.color}}>{r.value}</div>
    ...
    <div style={{marginBottom:40}}>
      <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:12}}>
        {TABS.map(function(t){var a=tab===t.id;return(
          <button key={t.id} onClick={function(){setTab(t.id);}} style={{padding:"6px 14px",borderRadius:16,border:"1.5px solid "+(a?C.teal:"rgba(28,31,46,0.2)"),background:a?C.teal:"transparent",color:a?C.slate:C.slate,fontFamily:"'DM Sans',sans-serif",fontSize:12,fontWeight:a?500:300,cursor:"pointer",display:"flex",alignItems:"center",gap:5}}>
      ...
      <div style={{display:"flex",alignItems:"flex-start",gap:8,background:TF,border:"1px solid rgba(0,212,180,0.25)",borderRadius:8,padding:"10px 14px",marginBottom:16}}>
        <span style={{color:C.teal,fontSize:13,marginTop:1}}>💡</span>
        <span style={{fontFamily:"'DM Sans',sans-serif",fontSize:12,fontWeight:300,color:C.slate}}>{desc}</span>
      </div>
      {tab==="pl"&&<PLStatement/>}
      {tab==="amz"&&<AmazonPL/>}
      {tab==="bs"&&<BSStatement/>}
...
function ToolCallout(props){
  ...
  return(
    <div style={{marginBottom:52}}>
      ...
      <h3 style={{fontFamily:"'Syne',serif",fontWeight:600,fontSize:22,color:C.slate,marginBottom:8}}>{isBs?"Explore a Real Balance Sheet":"Explore a Real Cash Flow Statement"}</h3>
      <p style={{fontFamily:"'DM Sans',sans-serif",fontWeight:300,fontSize:14,color:C.slate,opacity:0.65,marginBottom:20,lineHeight:1.75}}>{isBs?"Switch between Spec House Mid-Build, Post-Flip, and Amazon Store presets. Change any number and watch the equation respond — exactly what QuickBooks shows.":"Switch presets to see how cash moves differently across business types. This statement explains why profitable businesses sometimes run out of money."}</p>
      <div style={{background:C.mid,borderRadius:14,padding:"24px 28px"}}>
        <div style={{display:"flex",alignItems:"center",gap:16,flexWrap:"wrap"}}>
          <div style={{fontSize:32}}>{isBs?"⚖️":"💵"}</div>
          <div style={{flex:1}}>
            <div style={{fontFamily:"'Syne',serif",fontWeight:600,fontSize:16,color:C.ivory,marginBottom:4}}>{isBs?"Balance Sheet Explorer":"Cash Flow Explorer"}</div>
            <div style={{fontFamily:"'DM Sans',sans-serif",fontSize:13,fontWeight:300,color:C.ivory,opacity:0.65}}>{isBs?"Presets: Mid-Build · Post-Flip · Amazon · Blank":"Presets: Spec House Year · Completed Flip · Amazon · Blank"}</div>
          </div>
          <div style={{background:TF,border:"1px solid "+C.teal,borderRadius:8,padding:"10px 18px",textAlign:"center",flexShrink:0}}>
            <div style={{fontFamily:"'DM Sans',sans-serif",fontSize:10,fontWeight:500,letterSpacing:"0.15em",textTransform:"uppercase",color:C.teal,marginBottom:3}}>Open from</div>
            <div style={{fontFamily:"'DM Sans',sans-serif",fontSize:12,color:C.ivory}}>Sidebar → Tools</div>
          </div>
        </div>
      </div>
    </div>
...
function Certificate(){
  return(
    <div style={{marginTop:52,background:C.mid,borderRadius:20,padding:"44px 40px",textAlign:"center",border:"2px solid "+C.teal}}>
      ...
      <h2 style={{fontFamily:"'Syne',serif",fontWeight:600,fontSize:28,color:C.ivory,letterSpacing:"-0.02em",marginBottom:8}}>Money in Motion</h2>
      <p style={{fontFamily:"'DM Sans',sans-serif",fontWeight:300,fontSize:15,color:C.ivory,opacity:0.65,lineHeight:1.75,maxWidth:420,margin:"0 auto 24px"}}>You've completed all 14 modules. You can read financial statements, speak the language of capital, and understand the numbers behind every deal.</p>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10,maxWidth:440,margin:"0 auto"}}>
        {["14 Modules","3 Statement Types","Real-World Application"].map(function(item){return(<div key={item} style={{background:"rgba(0,212,180,0.1)",borderRadius:8,padding:"10px 6px"}}><div style={{fontFamily:"'DM Sans',sans-serif",fontSize:11,color:C.teal,fontWeight:400,lineHeight:1.4}}>{item}</div></div>);})}
      </div>
    </div>
  );
}
...
var ALL_MODS=[
  ...
  { id:2, label:"Module 2 · 30 min", title:"The People Behind\nthe Numbers",
    ...
    yt:{ title:"Bookkeeper vs. Accountant vs. CPA", ch:"Accounting Stuff", why:"Breaks down exactly the distinctions in Module 2. Watch to reinforce who does what and which one you need.", url:"https://www.youtube.com/@AccountingStuff/search?query=bookkeeper+accountant+CPA+difference" },
  },
  { id:3, label:"Module 3 · 30 min", title:"The Accounting\nFramework",
    ...
    yt:{ title:"Debits and Credits & the Accounting Equation", ch:"Accounting Stuff", why:"One of the best explanations of debits, credits, and the accounting equation available. Watch it and Module 3's framework will click.", url:"https://www.youtube.com/@AccountingStuff/search?query=debits+credits+accounting+equation" },
  },
  { id:4, label:"Module 4 · 30 min", title:"Assets —\nWhat You Own",
    ...
    yt:{ title:"Assets — Current vs. Fixed Assets Explained", ch:"Accounting Stuff", why:"A clear visual walkthrough of current vs. fixed assets with real-world examples. Great for reinforcing Module 4 before moving on.", url:"https://www.youtube.com/@AccountingStuff/search?query=current+assets+fixed+assets+balance+sheet" },
  },
  { id:5, label:"Module 5 · 30 min", title:"Liabilities & Equity\n— What You Owe & Own",
    ...
    yt:{ title:"Liabilities and Equity Explained", ch:"Accounting Stuff", why:"Covers current and long-term liabilities, equity, and the right side of the balance sheet — exactly what Module 5 introduced.", url:"https://www.youtube.com/@AccountingStuff/search?query=liabilities+equity+balance+sheet" },
  },
  { id:6, label:"Module 6 · 30 min", title:"Revenue, Costs\n& Expenses",
    ...
    yt:{ title:"Revenue, Expenses and Profit Explained", ch:"Accounting Stuff", why:"Walks through revenue, COGS, gross profit, and net income clearly. Perfect reinforcement before you read a full P&L in Module 7.", url:"https://www.youtube.com/@AccountingStuff/search?query=revenue+expenses+profit+income+statement" },
  },
  { id:7, label:"Module 7 · 30 min", title:"Reading the P&L",
    ...
    yt:{ title:"How to Read a Profit and Loss Statement", ch:"Accounting Stuff", why:"A clear walkthrough of a real P&L statement — exactly what Module 7 covered. See the structure come alive with real numbers.", url:"https://www.youtube.com/@AccountingStuff/search?query=how+to+read+profit+loss+income+statement" },
  },
  { id:8, label:"Module 8 · 30 min", title:"Reading the\nBalance Sheet",
    ...
    yt:{ title:"How to Read a Balance Sheet", ch:"Accounting Stuff", why:"A clear walkthrough of a real balance sheet — assets, liabilities, equity, and what each section tells you. Exactly what Module 8 covered, with visuals.", url:"https://www.youtube.com/@AccountingStuff/search?query=how+to+read+balance+sheet" },
  },
  ...
  { id:9, label:"Module 9 · 30 min", title:"Reading the Cash\nFlow Statement",
    ...
    yt:{ title:"How to Read a Cash Flow Statement", ch:"Accounting Stuff", why:"The clearest explanation of all three cash flow sections available. Watch after Module 9 — the operating activities adjustments will click in a new way.", url:"https://www.youtube.com/@AccountingStuff/search?query=how+to+read+cash+flow+statement" },
  },
  { id:10, label:"Module 10 · 30 min", title:"Accounting for\nReal Estate Projects",
    ...
    yt:{ title:"Real Estate Accounting — Job Costing for Flippers", ch:"Bigger Pockets / Real Estate Accounting Pros", why:"Covers project-level accounting for real estate investors — exactly the job costing framework Module 10 introduced. Practical and directly applicable.", url:"https://www.youtube.com/results?search_query=real+estate+job+costing+accounting+house+flip" },
  },
  ...
  { id:11, label:"Module 11 · 30 min", title:"Accounting for\nProduct-Based Business",
    ...
    yt:{ title:"Amazon FBA Accounting — How to Track Your Numbers", ch:"Accounting Stuff / FBA accounting channels", why:"Covers the Amazon-specific P&L structure, COGS calculation, and the cash flow dynamics that Module 11 introduced. Watch before setting up your first tracking system.", url:"https://www.youtube.com/results?search_query=amazon+fba+accounting+bookkeeping+COGS+beginners" },
  },
  ...
  { id:12, label:"Module 12 · 30 min", title:"Tax Basics for\nSmall Business",
    ...
    yt:{ title:"LLC vs S-Corp — Tax Differences Explained", ch:"Accounting Stuff / ClearValue Tax", why:"Covers entity structure and the S-Corp tax election clearly. Directly reinforces the most important decision new business owners face in Module 12.", url:"https://www.youtube.com/results?search_query=LLC+vs+S-Corp+tax+differences+small+business+explained" },
  },
  { id:13, label:"Module 13 · 30 min", title:"Accounting Tools\n& Technology",
    ...
    yt:{ title:"QuickBooks Online Tutorial for Small Business", ch:"QuickBooks", why:"The official QuickBooks walkthrough covers exactly the setup you need — chart of accounts, bank connections, and running basic reports. Watch this before you open your first account.", url:"https://www.youtube.com/results?search_query=QuickBooks+online+tutorial+small+business+beginners" },
  },
  ...
  { id:14, label:"Module 14 · 45 min", title:"Putting It\nAll Together",
    ...
    yt:{ title:"Complete Accounting Overview for Small Business Owners", ch:"Accounting Stuff", why:"A comprehensive review of everything this course covered — financial statements, key concepts, and how it all connects. The perfect final watch to consolidate everything you've learned.", url:"https://www.youtube.com/@AccountingStuff/search?query=accounting+overview+small+business+financial+statements" },
  },
];
...
export default function Course(){
  ...
  function goMod(id){setMid(id);setView("mod");}
  ...
  var TA={width:"100%",padding:"14px 16px",borderRadius:10,border:"1.5px solid rgba(28,31,46,0.1)",background:C.ivory,fontFamily:"'DM Sans',sans-serif",fontSize:14,fontWeight:300,color:C.slate,resize:"vertical",lineHeight:1.75};
  ...
  function Home(){return(
    <div style={{padding:"52px 52px 80px 0"}}>
      <div style={{marginBottom:52}}>
        ...
        <p style={{fontFamily:"'DM Sans',sans-serif",fontWeight:300,fontSize:17,color:C.slate,opacity:0.68,maxWidth:500,lineHeight:1.8}}>A practical accounting course built for entrepreneurs.</p>
      </div>
      {courseComplete&&<div style={{background:TF,border:"1.5px solid "+C.teal,borderRadius:14,padding:"20px 24px",marginBottom:32,maxWidth:560}}><div style={{display:"flex",alignItems:"center",gap:12}}><span style={{fontSize:24}}>🎓</span><div><div style={{fontFamily:"'Syne',serif",fontWeight:600,fontSize:16,color:C.teal,marginBottom:2}}>Course Complete!</div><div style={{fontFamily:"'DM Sans',sans-serif",fontSize:13,fontWeight:300,color:C.slate,opacity:0.7}}>All 14 modules finished. The tools and glossary are yours to use anytime.</div></div></div></div>}
      <div style={{marginBottom:48}}>
        <div style={{display:"flex",justifyContent:"space-between",marginBottom:10}}><span style={{fontFamily:"'DM Sans',sans-serif",fontSize:10,fontWeight:500,letterSpacing:"0.12em",color:C.slate,opacity:0.45,textTransform:"uppercase"}}>Your Progress</span><span style={{fontFamily:"'DM Sans',sans-serif",fontSize:12,color:C.teal,fontWeight:500}}>{done.length} / {MLIST.length} complete</span></div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,maxWidth:740}}>
        {MLIST.map(function(m){var d=done.includes(m.id);return(
          <div key={m.id} onClick={function(){if(m.on)goMod(m.id);}} onMouseEnter={function(e){if(m.on)e.currentTarget.style.transform="translateY(-1px)";}} onMouseLeave={function(e){e.currentTarget.style.transform="translateY(0)";}} style={{background:d?TF:C.dark,border:"1.5px solid "+(d?C.teal:m.on?"rgba(28,31,46,0.13)":"transparent"),borderRadius:14,padding:"20px 22px",cursor:m.on?"pointer":"default",opacity:m.on||d?1:0.38,transition:"all 0.2s"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}><span style={{fontFamily:"'DM Sans',sans-serif",fontSize:10,fontWeight:500,letterSpacing:"0.12em",textTransform:"uppercase",color:d?C.teal:C.slate,opacity:d?1:0.42}}>Module {m.id}</span>{d?<span style={{color:C.teal,fontSize:13}}>✓</span>:(!m.on&&<span style={{fontSize:11,opacity:0.3}}>🔒</span>)}</div>
            <p style={{fontFamily:"'Syne',serif",fontWeight:600,fontSize:13,color:C.slate,lineHeight:1.45}}>{m.title}</p>
            <p style={{fontFamily:"'DM Sans',sans-serif",fontSize:11,color:C.slate,opacity:0.38,marginTop:5}}>{m.id===14?"45 min":"30 min"}</p>
          </div>
        );})}
      </div>
    </div>
        ...
        <div style={{padding:"20px 52px 16px",borderBottom:"1px solid "+C.dark,flexShrink:0}}>
          ...
          <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
            {TOOLS.map(function(t){var a=activeTool===t.id;return(
              <button key={t.id} onClick={function(){setView("tool-"+t.id);}} style={{padding:"7px 14px",borderRadius:8,border:"1.5px solid "+(a?C.teal:"rgba(28,31,46,0.15)"),background:a?TF:"transparent",fontFamily:"'DM Sans',sans-serif",fontSize:13,color:a?C.teal:C.slate,cursor:"pointer",display:"flex",alignItems:"center",gap:6}}>
                <span>{t.icon}</span><span>{t.label}</span>
              </button>
            );})}
          </div>
  ...
  function ModView(){
    ...
    return(
      <div style={{padding:"52px 52px 100px 0",maxWidth:700}}>
        <button onClick={function(){setView("home");}} style={{fontFamily:"'DM Sans',sans-serif",fontSize:13,color:C.teal,background:"none",border:"none",cursor:"pointer",padding:0,marginBottom:24}}>← Back to Course</button>
        <div style={{marginBottom:52}}>
          <div style={EB}>{mod.label}</div>
          <h1 style={{fontFamily:"'Syne',serif",fontWeight:600,fontSize:42,letterSpacing:"-0.02em",color:C.slate,lineHeight:1.1,marginBottom:18,whiteSpace:"pre-line"}}>{mod.title}</h1>
          <p style={{fontFamily:"'DM Sans',sans-serif",fontWeight:300,fontSize:16,color:C.slate,opacity:0.65,lineHeight:1.75}}>{mod.intro}</p>
          {!isCapstone&&<div style={{display:"inline-flex",alignItems:"center",gap:8,marginTop:18,background:TF,borderRadius:8,padding:"8px 14px"}}><span style={{color:C.teal}}>💡</span><span style={{fontFamily:"'DM Sans',sans-serif",fontSize:12,color:C.teal}}>Tap any <strong style={{borderBottom:"1.5px solid "+C.teal}}>teal underlined term</strong> to see its definition</span></div>}
        </div>
        {mod.sections.map(function(s,si){
          ...
          return(
            <div key={si} style={{marginBottom:48}}>
              <h2 style={{fontFamily:"'Syne',serif",fontWeight:600,fontSize:19,color:C.slate,marginBottom:18,paddingBottom:14,borderBottom:"1px solid "+C.dark}}>{s.title}</h2>
              {isCapstoneFin
                ? <div><p style={{fontFamily:"'DM Sans',sans-serif",fontWeight:300,fontSize:16,color:C.slate,lineHeight:1.9,marginBottom:24}}>{s.paras[0]}</p><CapstoneTabs/>{s.paras.slice(1).map(function(p,pi){return <Para key={pi} text={p} onTerm={setTerm}/>;})}</div>
        ...
        <div style={{marginBottom:52}}>
          ...
          <h3 style={{fontFamily:"'Syne',serif",fontWeight:600,fontSize:24,color:C.slate,marginBottom:10}}>Before You Continue</h3>
          <p style={{fontFamily:"'DM Sans',sans-serif",fontWeight:300,fontSize:14,color:C.slate,opacity:0.6,marginBottom:28,lineHeight:1.7}}>Not graded — just for you. Writing it down makes it stick.</p>
          {mod.cfu.map(function(q,qi){var k=mod.id+"-"+qi;return(
            <div key={qi} style={{background:C.dark,borderRadius:14,padding:"22px 24px",marginBottom:20}}>
              <p style={{fontFamily:"'DM Sans',sans-serif",fontWeight:400,fontSize:15,color:C.slate,lineHeight:1.75,marginBottom:16}}>{q}</p>
              ...
              <button onClick={function(){if(cfuDraft.current[k]!==undefined){setCfuA(function(p){var n=Object.assign({},p);n[k]=cfuDraft.current[k];return n;});}setCfuS(function(p){var n=Object.assign({},p);n[k]=true;return n;});}} style={{marginTop:10,padding:"7px 18px",background:cfuS[k]?C.teal:"transparent",color:cfuS[k]?C.slate:C.teal,border:"1.5px solid "+C.teal,borderRadius:7,fontFamily:"'DM Sans',sans-serif",fontSize:13,fontWeight:500,cursor:"pointer",transition:"all 0.2s"}}>{cfuS[k]?"✓ Saved":"Save Answer"}</button>
            </div>
          );})}
        </div>
        <div style={{marginBottom:52}}>
          <div style={EB}>{isCapstone?"Capstone Assessment":"Module Assessment"}</div>
          <h3 style={{fontFamily:"'Syne',serif",fontWeight:600,fontSize:24,color:C.slate,marginBottom:8}}>{mod.assessment.title}</h3>
          <p style={{fontFamily:"'DM Sans',sans-serif",fontWeight:300,fontSize:14,color:C.slate,opacity:0.6,marginBottom:24,lineHeight:1.7}}>{isCapstone?"This is the final assessment — six questions covering two real business meetings. Take your time. Submit when ready for your tutor's full review.":"Read the scenario and respond in your own words. Submit for personalized tutor feedback."}</p>
          <div style={{background:C.mid,borderRadius:14,padding:"24px 28px",marginBottom:24}}>
            ...
            <div style={{fontFamily:"'DM Sans',sans-serif",fontSize:14,fontWeight:300,color:C.ivory,lineHeight:1.9,whiteSpace:"pre-line"}}>{mod.assessment.scenario}</div>
          </div>
          ...
          {asmL&&<div style={{display:"flex",alignItems:"center",gap:12,marginTop:16,padding:"14px 20px",background:"rgba(28,31,46,0.04)",borderRadius:10,border:"1px solid rgba(28,31,46,0.08)"}}>
            ...
            <span style={{fontFamily:"'DM Sans',sans-serif",fontSize:13,fontWeight:300,color:"#1C1F2E",opacity:0.6}}>Your tutor is reviewing your response — this usually takes 15–30 seconds.</span>
          </div>}
          {af&&<div style={{marginTop:28,background:C.dark,borderLeft:"4px solid "+C.teal,borderRadius:"0 14px 14px 0",padding:"24px 28px"}}><div style={EB}>Tutor Feedback</div><div style={{fontFamily:"'DM Sans',sans-serif",fontSize:15,fontWeight:300,color:C.slate,lineHeight:1.85,whiteSpace:"pre-wrap",marginTop:8}}>{af}</div></div>}
        </div>
        <div style={{background:C.mid,borderRadius:18,padding:"28px 32px",marginBottom:52}}>
          ...
          <h3 style={{fontFamily:"'Syne',serif",fontWeight:600,fontSize:18,color:C.ivory,marginBottom:4}}>{mod.yt.title}</h3>
          <p style={{fontFamily:"'DM Sans',sans-serif",fontSize:12,color:C.teal,marginBottom:14}}>{mod.yt.ch}</p>
          <p style={{fontFamily:"'DM Sans',sans-serif",fontSize:14,fontWeight:300,color:C.ivory,opacity:0.75,lineHeight:1.75,marginBottom:22}}>{mod.yt.why}</p>
          <button onClick={function(){window.open(mod.yt.url,"_blank");}} style={{display:"inline-flex",alignItems:"center",gap:8,background:C.teal,color:C.slate,padding:"10px 22px",borderRadius:8,fontFamily:"'DM Sans',sans-serif",fontSize:13,fontWeight:500,border:"none",cursor:"pointer"}}>▶ Watch on YouTube</button>
        </div>
        {done.includes(mod.id)
          ?<div>{isCapstone&&<Certificate/>}<div style={{textAlign:"center",padding:24,color:C.teal,fontFamily:"'DM Sans',sans-serif",fontSize:15,fontWeight:500}}>✓ Module {mod.id} Complete{isCapstone?" — Course finished. Well done.":""}</div></div>
          :<button onClick={function(){setDone(function(p){return p.concat([mod.id]);});setView("home");}} onMouseEnter={function(e){e.currentTarget.style.opacity="0.88";}} onMouseLeave={function(e){e.currentTarget.style.opacity="1";}} style={{width:"100%",padding:16,background:C.teal,color:C.slate,border:"none",borderRadius:12,fontFamily:"'Syne',serif",fontSize:16,fontWeight:600,cursor:"pointer"}}>{isCapstone?"Complete the Course →":"Mark Module "+mod.id+" Complete →"}</button>
        }
      </div>
    );
  }
  ...
  function GlossView(){return(
    <div style={{padding:"52px 52px 100px 0"}}>
      <button onClick={function(){setView("home");}} style={{fontFamily:"'DM Sans',sans-serif",fontSize:13,color:C.teal,background:"none",border:"none",cursor:"pointer",padding:0,marginBottom:24}}>← Back</button>
      <div style={EB}>Course Reference</div>
      <h1 style={{fontFamily:"'Syne',serif",fontWeight:600,fontSize:42,letterSpacing:"-0.02em",color:C.slate,marginBottom:40}}>Glossary</h1>
      <div style={{display:"grid",gap:12}}>
        {Object.entries(GL).sort(function(a,b){return a[0].localeCompare(b[0]);}).map(function(e){return(
          <div key={e[0]} style={{background:C.dark,borderRadius:12,padding:"18px 22px"}}>
            <p style={{fontFamily:"'Syne',serif",fontWeight:600,fontSize:15,color:C.teal,marginBottom:6}}>{dsp(e[0])}</p>
            <p style={{fontFamily:"'DM Sans',sans-serif",fontWeight:300,fontSize:14,color:C.slate,lineHeight:1.75}}>{e[1]}</p>
          </div>
        );})}
      </div>
    </div>
      ...
      <div style={{display:"flex",height:"100vh",background:C.ivory,overflow:"hidden"}}>
        <div style={{width:bar?288:0,minWidth:bar?288:0,background:C.slate,height:"100vh",overflow:"hidden",transition:"all 0.3s",flexShrink:0}}>
          <div style={{width:288,height:"100%",overflowY:"auto",padding:"28px 20px"}}>
            <div style={{marginBottom:28,paddingLeft:4,display:"flex",alignItems:"center",gap:4}}><span style={{fontFamily:"'Syne',serif",fontWeight:600,fontSize:14,color:C.ivory,letterSpacing:"0.04em"}}>Money in Motion</span><span style={{color:C.teal,fontSize:18}}>·</span></div>
            <div style={{marginBottom:24,paddingLeft:4}}>
              <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}><span style={{fontFamily:"'DM Sans',sans-serif",fontSize:10,fontWeight:500,letterSpacing:"0.12em",color:C.ivory,opacity:0.38,textTransform:"uppercase"}}>Progress</span><span style={{fontFamily:"'DM Sans',sans-serif",fontSize:11,color:C.teal}}>{pct}%</span></div>
            ...
            {[{l:"⌂  Dashboard",v:"home"},{l:"⊞  Glossary",v:"gloss"}].map(function(x){return(<button key={x.v} onClick={function(){setView(x.v);}} style={{display:"block",width:"100%",textAlign:"left",padding:"9px 12px",marginBottom:2,borderRadius:8,background:view===x.v?TF:"transparent",border:"none",cursor:"pointer",fontFamily:"'DM Sans',sans-serif",fontSize:13,color:view===x.v?C.teal:C.ivory}}>{x.l}</button>);})}
            ...
            {TOOLS.map(function(t){var active=view==="tool-"+t.id;return(
              <button key={t.id} onClick={function(){setView("tool-"+t.id);}} style={{display:"flex",alignItems:"center",gap:8,width:"100%",textAlign:"left",padding:"7px 12px",borderRadius:8,marginBottom:1,background:active?TF:"transparent",border:"none",cursor:"pointer"}}>
                ...
                <div><div style={{fontFamily:"'DM Sans',sans-serif",fontSize:12,color:active?C.teal:C.ivory,opacity:active?1:0.75}}>{t.label}</div><div style={{fontFamily:"'DM Sans',sans-serif",fontSize:10,color:C.ivory,opacity:0.35}}>{t.desc}</div></div>
              </button>
            );})}
            ...
            {MLIST.map(function(m){var d=done.includes(m.id),cur=view==="mod"&&mid===m.id;return(
              <button key={m.id} onClick={function(){if(m.on)goMod(m.id);}} style={{display:"flex",alignItems:"flex-start",gap:10,width:"100%",textAlign:"left",padding:"7px 12px",borderRadius:8,marginBottom:1,background:cur?TF:"transparent",border:"none",cursor:m.on?"pointer":"default",opacity:m.on||d?1:0.28}}>
                ...
                <span style={{fontFamily:"'DM Sans',sans-serif",fontSize:12,color:cur||d?C.teal:C.ivory,lineHeight:1.4,opacity:cur?1:0.75}}>{m.id}. {m.title}</span>
              </button>
            );})}
          </div>
        </div>
        <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
          <div style={{background:C.ivory,borderBottom:"1px solid "+C.dark,padding:"14px 52px",display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0}}>
            <button onClick={function(){setBar(function(b){return !b;});}} style={{background:"none",border:"none",cursor:"pointer",fontSize:17,color:C.slate,opacity:0.45,padding:"2px 4px"}}>☰</button>
            <div style={{display:"flex",gap:8}}>
              <button onClick={function(){setView("tool-pl");}} style={{fontFamily:"'DM Sans',sans-serif",fontSize:12,color:C.teal,background:"none",border:"1px solid rgba(0,212,180,0.4)",borderRadius:6,padding:"6px 12px",cursor:"pointer"}}>📊 Tools</button>
              <button onClick={function(){setView("gloss");}} style={{fontFamily:"'DM Sans',sans-serif",fontSize:12,color:C.teal,background:"none",border:"1px solid rgba(0,212,180,0.4)",borderRadius:6,padding:"6px 12px",cursor:"pointer"}}>Glossary</button>
            </div>
          </div>
        ...
        {term&&(<div onClick={function(){setTerm(null);}} style={{position:"fixed",inset:0,background:"rgba(28,31,46,0.55)",zIndex:300,display:"flex",alignItems:"center",justifyContent:"center"}}><div onClick={function(e){e.stopPropagation();}} style={{background:C.ivory,borderRadius:18,padding:"32px 36px",maxWidth:420,margin:24,boxShadow:"0 24px 80px rgba(28,31,46,0.3)"}}><div style={EB}>Definition</div><h3 style={{fontFamily:"'Syne',serif",fontWeight:600,fontSize:24,color:C.slate,marginBottom:14}}>{dsp(term)}</h3><p style={{fontFamily:"'DM Sans',sans-serif",fontSize:15,fontWeight:300,color:C.slate,lineHeight:1.85}}>{GL[term]}</p><button onClick={function(){setTerm(null);}} style={{marginTop:22,padding:"9px 22px",background:C.teal,color:C.slate,border:"none",borderRadius:8,fontFamily:"'DM Sans',sans-serif",fontSize:13,fontWeight:500,cursor:"pointer"}}>Got it</button></div></div>)}
        <div style={{position:"fixed",bottom:24,right:24,zIndex:200}}>
          {chat&&(<div style={{position:"absolute",bottom:68,right:0,width:348,height:460,background:C.ivory,borderRadius:18,boxShadow:"0 20px 60px rgba(28,31,46,0.22)",display:"flex",flexDirection:"column",overflow:"hidden",border:"1px solid "+C.dark}}>
            <div style={{background:C.slate,padding:"15px 18px",display:"flex",justifyContent:"space-between",alignItems:"center",flexShrink:0}}>
              <div><div style={{fontFamily:"'Syne',serif",fontWeight:600,fontSize:14,color:C.ivory}}>Ask Your Tutor</div><div style={{fontFamily:"'DM Sans',sans-serif",fontSize:11,color:C.teal,marginTop:2}}>{mod?"Module "+mod.id+" · "+mod.title.replace("\n"," "):"Money in Motion"}</div></div>
              <button onClick={function(){setChat(false);}} style={{background:"none",border:"none",color:C.ivory,opacity:0.5,cursor:"pointer",fontSize:22,lineHeight:1}}>×</button>
            </div>
            <div style={{flex:1,overflowY:"auto",padding:"14px 16px",display:"flex",flexDirection:"column",gap:10}}>
              {msgs.length===0&&<div style={{textAlign:"center",padding:"28px 12px"}}><div style={{fontSize:28,marginBottom:10}}>💬</div><p style={{fontFamily:"'DM Sans',sans-serif",fontSize:13,color:C.slate,opacity:0.5,lineHeight:1.7}}>Got a question? Ask anything — no question is too basic.</p></div>}
              {msgs.map(function(m,i){return(<div key={i} style={{display:"flex",justifyContent:m.role==="user"?"flex-end":"flex-start"}}><div style={{maxWidth:"82%",padding:"10px 13px",borderRadius:m.role==="user"?"14px 14px 3px 14px":"14px 14px 14px 3px",background:m.role==="user"?C.teal:C.dark,fontFamily:"'DM Sans',sans-serif",fontSize:13,fontWeight:300,color:C.slate,lineHeight:1.65}}>{m.content}</div></div>);})}
              {cL&&<div style={{display:"flex"}}><div style={{background:C.dark,padding:"10px 14px",borderRadius:"14px 14px 14px 3px",fontFamily:"'DM Sans',sans-serif",fontSize:13,color:C.slate,opacity:0.45}}>Thinking...</div></div>}
              <div ref={endRef}/>
            </div>
            <div style={{padding:10,borderTop:"1px solid "+C.dark,display:"flex",gap:8,flexShrink:0}}>
              <input value={inp} onChange={function(e){setInp(e.target.value);}} onKeyDown={function(e){if(e.key==="Enter"&&!e.shiftKey)sendChat();}} placeholder="Ask a question..." style={{flex:1,padding:"9px 13px",borderRadius:8,border:"1.5px solid "+C.dark,background:C.dark,fontFamily:"'DM Sans',sans-serif",fontSize:13,color:C.slate}}/>
              <button onClick={sendChat} disabled={!inp.trim()||cL} style={{padding:"9px 14px",background:inp.trim()?C.teal:C.dark,border:"none",borderRadius:8,cursor:inp.trim()?"pointer":"default",fontSize:15,transition:"background 0.2s"}}>→</button>
            </div>
          </div>)}
